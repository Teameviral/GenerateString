...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nTolong Laporan ke @SharingUserbot jika eror " \
...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\n𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲𝗱 𝗯𝘆 @StringKangGhosting_Bot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Berhasil Megambil {} string session. \n\nSilahkan cek di Pesan Tersimpan/Saved Message! \n\nBy @Mafia_TobaTZ".format("telethon" if telethon else "pyrogram"))
